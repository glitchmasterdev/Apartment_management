  ...
  try {
    ...
    if (tbody) {
      ...
      mine.slice().reverse().forEach(p => {
        ...
        const isVerified = ['safaricom_confirmed', 'stk_confirmed'].includes(p.verification_status);
        let actionBtns = isVerified ? `<button class="receipt-btn" data-payment='${encodeURIComponent(JSON.stringify(p))}' type="button">Print verified receipt</button>` : '';
...
function downloadReceipt(payment) { if (!['safaricom_confirmed','stk_confirmed'].includes(payment.verification_status)) { window.showToast('A receipt is available only after Safaricom confirmation.', 'error'); return; } const user=window.getCurrentUser()||{}; const receipt=window.open('', '_blank'); receipt.document.write(`<html><head><title>Verified Payment Receipt</title><style>body{font:16px Arial;padding:32px}h1{color:#c2593f}@media print{button{display:none}}</style></head><body><h1>Verified Payment Receipt</h1><p><b>Tenant:</b> ${user.full_name||''}<br><b>Account:</b> ${user.account_number||''}<br><b>M-Pesa code:</b> ${payment.mpesa_code||''}<br><b>Amount:</b> KES ${Number(payment.amount_paid||payment.amount||0).toLocaleString()}<br><b>Date:</b> ${payment.payment_date||''}<br><b>Verification:</b> Confirmed by Safaricom</p><button onclick="window.print()">Print</button></body></html>`); receipt.document.close(); }
